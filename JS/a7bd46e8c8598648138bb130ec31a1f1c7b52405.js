...
function showContent(){
    ...
    switch (finalSection){
      case 'downloads':
        ...
        getInventory();
        downloadPage.style.opacity = 1.00;
      ...
      case 'processing':
        ...
        processingPage.style.opacity = 1.00;
        ...
        message.innerHTML = '<h2>processing your purchase...</h2>Confirming stock and retrieving your download link.';
...
function updateProcessingPage(status, response){
  ...
  var newMessage;
  switch (status){
    case 200: // Processed successfully
      newMessage = "<h2>thank you <3</h2>Download your music at the link below (a copy has also been emailed to you).<br><br><div style='text-align:center;'><a class='btn' style='max-width:25vh' target='_blank' href="+response.downloadLink+">Download!</a></div><br><br>If you encounter any problems, don't hesitate to get in contact at <a href='"+decodeEmail('znvygb:qlyna@ybsvtrarengbe.pbz')+"'>"+decodeEmail('qlyna@ybsvtrarengbe.pbz')+"</a><br><br><b>Your order reference is "+response.orderRef+".</b>";
      break;
    case 210: // Already processed (user probably reloaded the page)
      newMessage = "<h2>thank you <3</h2>Your download link has been emailed to the address you provided at checkout. <b>Your order reference is "+response.orderRef+".</b><br><br>If you encounter any problems, don't hesitate to get in contact at <a href='"+decodeEmail('znvygb:qlyna@ybsvtrarengbe.pbz')+"'>"+decodeEmail('qlyna@ybsvtrarengbe.pbz')+"</a>";
      break;
    case 211: // Out of stock
      newMessage = "<h2>sorry!</h2>Your order couldn't be fulfilled because this item is now out of stock.<br><br>Check back again soon or consider the <a style='text-decoration:underline' target='_blank' href='https://dylanturner.bandcamp.com/album/generated-beats-volume-1'>album on bandcamp</a> if you need tracks in a hurry and don't mind that they're not unique.<br><br>You haven't been charged and your funds will be automatically released.";
      break;
    case 400: // Invalid request
    default:
      newMessage = "<h2>sorry!</h2>Something went wrong. This usually happens if you visit this page without successfully completing the checkout first.<br><br>If you're trying to make a purchase please try again or <a style='text-decoration:underline' href='"+decodeEmail('znvygb:qlyna@ybsvtrarengbe.pbz')+"'>get in contact</a> if you think there's a problem with the site.";
  }
...
function updateVol(){
  volDial.style.transform = 'translateY(-50%) translateX(-50%) rotate('+vol/(100/270)+'deg)';
  ...
  volSpeaker.style.transform = vol < 1 ? 'translateY(-50%) translateX(-60%)' : 'translateY(-50%) translateX(-70%)';
...
function volDrag(x,y, startX, startY, startVol){
  ...
  var volChange = (moveDist / 0.25) * 100; // 1/4 screen height = 100% volume change
  vol = Math.max(Math.min((startVol + volChange), 100),0); // Clamp new volume between 0-100
  updateVol();
}
...
function moreExpand(){
  ...
  svg = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
  svg.setAttribute('viewBox', '0 0 100 100');
  svg.innerHTML = "<circle fill='#FFF' cy='50' cx='12' r='10'><animate/></circle>";
  svg.innerHTML += "<circle fill='#FFF' cy='50' cx='50' r='10'><animate/></circle>";
  svg.innerHTML += "<circle fill='#FFF' cy='50' cx='88' r='10'><animate/></circle>";
  svg.innerHTML += "<path d='M50 50 L50 50 L50 50' stroke='#FFF' stroke-width='20' fill='none' stroke-linejoin='round'><animate/></path>";
...
function moreClose(){
  ...
  svg.innerHTML = "<circle fill='#FFF' cy='65' cx='12' r='12'><animate/></circle>";
  svg.innerHTML += "<circle fill='#FFF' cy='35' cx='50' r='12'><animate/></circle>";
  svg.innerHTML += "<circle fill='#FFF' cy='65' cx='88' r='12'><animate/></circle>";
  svg.innerHTML += "<path d='M12 65 L50 35 L88 65' stroke='#FFF' stroke-width='18' fill='none' stroke-linejoin='round'><animate/></path>";
...
function initPlayStop(){
  svg = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
  svg.setAttribute('viewBox', '0 0 100 100');
  svg.innerHTML = "<path d='M15 15 L85 50 L85 50 L15 85 Z' stroke='#FFF' stroke-width='20' fill='#fff' stroke-linejoin='round'/>"
...
function stopToPlay(){
  ...
  svg = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
  svg.setAttribute('viewBox', '0 0 100 100');
  svg.innerHTML = "<path d='M10 10 L90 10 L90 90 L10 90 Z' stroke='#FFF' stroke-width='20' fill='#fff' stroke-linejoin='round'><animate/></path>"
...
function playToStop(){
  ...
  svg = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
  svg.setAttribute('viewBox', '0 0 100 100');
  svg.innerHTML = "<path d='M15 15 L85 50 L85 50 L15 85 Z' stroke='#FFF' stroke-width='20' fill='#fff' stroke-linejoin='round'><animate/></path>"
...
function backClick(){
  window.history.pushState({"pageTitle":'lofi generator - listen'},"", '/');
  document.title = 'lofi generator';
  aboutPage.style.opacity = 0;
  aboutPage.style.pointerEvents = 'none';
  downloadPage.style.opacity = 0;
...
function downloadClick(){
  getInventory();
  window.history.pushState({"pageTitle":'lofi generator - downloads'},"", '/downloads');
  document.title = 'lofi generator - Downloads';

  downloadPage.style.pointerEvents = 'auto';
  downloadPage.style.display = 'block';
  setTimeout(function(){downloadPage.style.opacity = 1.00;},0);
...
function pulseKick(time){
  Tone.Draw.schedule(function(){
    ...
    playStop.style.transition = 'width 0.1s, height 0.1s';
		playStop.style.width = playStop.style.height = 4-between(2,3)*vol/100+'vh'
	}, time)
...
function pulseSnare(time){
  Tone.Draw.schedule(function(){
    snareRing.style.transform = 'translateY(-50%) translateX(-50%) rotate('+between(0,180)+'deg)';
...
function pulseHat(time){
  Tone.Draw.schedule(function(){
    hatRing.style.transform = 'translateY(-50%) translateX(-50%) rotate('+between(0,180)+'deg)';
...
function pulseChordal(time){
  Tone.Draw.schedule(function(){
    chordalRing.style.transform = 'translateY(-50%) translateX(-50%) rotate('+between(0,180)+'deg)';
...
function initVisualiser(){
  if (chordalRing) {chordalRing.remove();}
...
function checkLoaded(song){
  var sounds = [kick.sampler,snare.sampler,hat.sampler, song.chordal,song.bass,song.lead,song.ambience,song.vinyl];
  for (var i = 0; i < sounds.length; i++) {
    if (!sounds[i]) { return false; }
    if (sounds[i].loaded === false) { return false; }
  }
  return true;
}
...
function getAudioBufferFromFilepath(cache, filepath){
  return new Promise(resolve => {
    ...
    } else { // cache isn't available, just fetch the file
      fetch(filepath)
...
function checkBuffersLoaded(map){ // Returns true if every value of the map has been overwritten with an audioBuffer
...
function getInventory() {
  ...
  var now = Date.now();
  if (inventoryUpdated > now - 300000){ return; }
  ...
  fetch('/.netlify/functions/check-stock', {
    ...
    if (res.status == 200){
      res.json().then(data => {
        ['LFU20','LFU100'].forEach(sku => updateStockIndicator(sku, data[sku]));
...
function updateStockIndicator(sku, stock) {
  ...
  if (stock){
    ...
    btn.onclick = function() { checkout(sku); };
  } else {
...
function chooseChordal(){
  var chordal = choose([rhodes1, rhodes0, piano0, piano1, kalimba, synth0, marimba]);

  return chordal.connect(chordalFilter);
}

function chooseLead(){
  var lead = choose([rhodes1, rhodes0, piano1, kalimba, synth0, dantranh, marimba, violinpizz, guitar, uke]);
...
function getBassRhythmic(prog, progRhythm, halfspeed){
  ...
  for (var i = 0; i < progRhythm.length; i++){
    let chordEighths = Math.round(progRhythm[i].toSeconds() / Tone.Time('8n').toSeconds());
    let chord = prog[i];
    let pos = 0;
...
function getHarmoniousNotes(scale, chord, intervals = [0,2,3,4,7,9,10,11]){
    ...
    for (var i = 0; i < possibilities.length; i++){
      if (scale.includes(possibilities[i]) && !harmonious.includes(possibilities[i])) {
...
function chooseNote(phrase, scale, chord, range, newChord) {
  ...
  // Choose an action
  let action = choose(['first','repeat','step','leap','rest'],probabilities);
  let note;
...
function getHarmony(key, prog, progRhythm, halfspeed, melody){
  let scale = [0,2,4,5,7,9,11].map(x => (x+key)%12);
  let lower = chance(0.7) ? true : false; // Whether to favour lower or higher harmonies
    ...
    if (!melody[i]){
      ...
      for (var j = 0; j < melody[i].length; j++){
        if (!melody[i][j]){
          ...
          let note = options.length > 0 ? choose(options) : choose([melodyNote+12, melodyNote-12]);
...
;
// globals
let gain, vibratoSlow, vibratoFast, lowpass, filter, chordalGain, chordalVerb,
...
function getSong(transport, offline = false, bpm = between(60,90)) {
  ...
  let introA = getSection(song, chooseSome(
    [playBlockChords, 1],
    [playMelody, 0.5],
  ...
  let introB = getSection(song, chooseSome(
  ...
  let sectionA = getSection(song, chooseSome(
    choose([[playBlockChords, 1], [playAltBlockChords, 1]]),
    [playMelody, 1],
    [playHarmony, 0.75],
  ...
  let sectionB = getSection(song, chooseSome(
    choose([[playBlockChords, 1], [playAltBlockChords, 1]]),
    [playMelody, 1],
    [playHarmony, 1],
  ...
  let sectionC = getSection(song, chooseSome(
    choose([[playBlockChords, 1], [playAltBlockChords, 1]]),
    [playAltMelody, 1],
    [playAltHarmony, 0.5],
    choose([[playBeat, 1],[playBeatFill, 1]],[0.6,0.4]),
  ...
  let sectionD = getSection(song, chooseSome(
    choose([[playBlockChords, 1], [playAltBlockChords, 1]]),
...
function triggerHumanised(instrument, notes, duration, time, velRange, maxDelay){
...
function duck(now, bassDucking){
  let attack = 0.008;
  let min = 0.015849; // -36db change from 1
  let release = 0.44;
  let sustain = 0.06;
...
function stop(){
  for (var i = 0; i < ambiences.length; i++){
    if (ambiences[i].player && ambiences[i].player.state == 'started') {ambiences[i].player.stop();}
  }
  for (var i = 0; i < vinyls.length; i++){
    if (vinyls[i].player && vinyls[i].player.state == 'started') {vinyls[i].player.stop();}
  }
